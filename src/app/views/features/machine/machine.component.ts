...
export class MachineComponent {
  ...
  addModel() {
    ...
    if (this.modelForm.valid) {
      ...
      this.HttpApi.postMachineRequest(this.modelForm.value).subscribe(Request => {
        console.log(Request)

        switch (Request.code) {
          case 200:
            this.messageService.add({ severity: 'success', summary: '成功', detail: '機台新增' });
            this.getMachine(1, 1000)
  ...
  deleteModel(id: any) {
    this.HttpApi.deleteMachineRequest(id).subscribe(Request => {
      console.log(Request)
  ...
  saveModel() {
    ...
    this.HttpApi.getMachineRequest(this.editData.file_id, this.editData)
      .subscribe(Request => {
        console.log(Request)
        switch (Request.code) {
          case 200:
